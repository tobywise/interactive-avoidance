"""
This script fits MCTS models using inverse binomial sampling (IBS).
"""
...
if __name__ == "__main__":
    ...
    # Get game info
    with open(
        "data/game_info/experiment-{0}/condition_{1}.json".format(
            run_params["experiment"], run_params["condition"]
        ),
        "r",
    ) as f:
    ...
    # Get response data
    if "simulation" in run_params and run_params["simulation"]:
        recovery = True
        response_data = pd.read_csv(
            "data/response_data/experiment-{0}/condition-{1}/exp-{0}_cond-{1}_response_data_SIMULATED.csv".format(
                run_params["experiment"], run_params["condition"]
            )
        )
    else:
        recovery = False
        response_data = pd.read_csv(
            "data/response_data/experiment-{0}/condition-{1}/exp-{0}_cond-{1}_response_data.csv".format(
                run_params["experiment"], run_params["condition"]
            )
        )
    ...
    # fit_settings['MCTS_iterations'] = 10
    # fit_settings['n_repeats'] = 1
    # fit_settings['stopping_point'] = 1
    ...
    for env in sorted(subject_response_data["env"].unique())[start_env:end_env]:
        ...
        sub_prey_df = prey_df[prey_df["env"] == env].sort_values(
            ["trial", "response_number"]
        )
        sub_predator_df = predator_df[predator_df["env"] == env].sort_values(
            ["trial", "response_number"]
        )

        # Check that everything is in order
        assert np.all(
            sub_prey_df.trial.diff()[1:] <= 1
        ), "Trials are not in order or there are trials missing, jumps of more than 1"
        assert np.all(
            sub_prey_df.trial.diff()[1:] >= 0
        ), "Trials are not in order or there are trials missing, jumps of less than 0"

        sub_prey_states = [envs[env].agents["Prey_1"].position] + sub_prey_df[
            "cellID"
        ].tolist()

        # Get true responses if using simulated data
        if "simulation" in run_params and run_params["simulation"]:
            true_sub_prey_states = [envs[env].agents["Prey_1"].position] + sub_prey_df[
                "true_response"
            ].astype(int).tolist()

        # Predator states = the state the predator ended up at (i.e. the final response number)
        sub_predator_states = [
            envs[env].agents["Predator_1"].position
        ] + sub_predator_df.loc[
            sub_predator_df["response_number"]
            == sub_predator_df["response_number"].max(),
            "cellID",
        ].tolist()

        sub_predator_states = sub_predator_states[
            :-1
        ]  # Remove final state as this is after the prey has made its final move
        ...
        if not len(sub_predator_states) == len(sub_prey_states) - 1:
            print(
                "Unequal number of predator & prey states, env {0}, subject {1}".format(
                    env, subject
                )
            )
            ...
            for n in range(run_params["n_moves"]):
                ...
                if "simulation" in run_params and run_params["simulation"]:
                    game_info["environments"][env]["agents"][1]["Prey_1"][
                        "position"
                    ] = true_sub_prey_states[n]
                else:
                    game_info["environments"][env]["agents"][1]["Prey_1"][
                        "position"
                    ] = sub_prey_states[n]
                ...
                try:
                    subject_env = hex_environment_from_dict(
                        game_info["environments"][env], ["Dirt", "Trees", "Reward"]
                    )
                    ...
                    if run_params["model"] == "MCTS_interactive":
                        subject_env.agents["Prey_1"].algorithm = MCTS(
                            interactive=True,
                            n_iter=fit_settings["MCTS_iterations"],
                            caching=True,
                            opponent_algorithm="value_iteration",
                    ...
                    # Null model, random action selection
                        ...
                        raise ValueError(
                            "{0} is an invalid model string".format(run_params["model"])
                        )
                    ...
                    print('Environment {0}, trial {1}'.format(env, n))
                    print(subject_env.agents['Predator_1'].position)
    ...
    for env in sorted(subject_response_data["env"].unique())[start_env:end_env]:
        ...
        ibs = IBSEstimator(
            n_jobs,
            fit_settings["n_repeats"],
            fit_settings["stopping_point"],
            run_params["max_planning_steps"],
        )
        ibs.fit(
            environments["Env_{0}".format(env)][: run_params["n_moves"]],
            "Prey_1",
            state_history["Env_{0}".format(env)]["prey"][1 : 1 + run_params["n_moves"]],
        )
    ...
    if "environments" in run_params:
        if recovery:
            output_path = "data/IBS_recovery_fits/experiment_{0}/condition_{1}/environments_{2}".format(
                run_params["experiment"],
                run_params["condition"],
                run_params["environments"],
            )
        else:
            output_path = (
                "data/IBS_fits/experiment_{0}/condition_{1}/environments_{2}".format(
                    run_params["experiment"],
                    run_params["condition"],
                    run_params["environments"],
                )
        ...
        if recovery:
            output_path = "data/IBS_recovery_fits/experiment_{0}/condition_{1}".format(
                run_params["experiment"], run_params["condition"]
            )
        else:
            output_path = "data/IBS_fits/experiment_{0}/condition_{1}".format(
                run_params["experiment"], run_params["condition"]
            )
    ...
    # Save to CSV
    fitting_output.to_csv(
        os.path.join(
            output_path,
            "jobID-{2}_subject-{0}_model-{1}.csv".format(
                subject, run_params["model"], runID
            ),
        ),
        index=False,
    )
